      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-center">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full rounded-lg border border-slate-800 bg-slate-950/80 py-2.5 pl-10 pr-4 text-sm text-slate-200 placeholder-slate-500 outline-none transition-all focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20"
          />
        </div>
        ...
        <div className="w-full md:w-56">
          <select value={filters.sort_by} onChange={(e) => setFilter("sort_by", e.target.value)} className="w-full rounded-lg border border-slate-800 bg-slate-950/80 px-3 py-2.5 text-sm text-slate-300 outline-none focus:border-indigo-500">
            <option value="performance_ratio">Ordenar: Breakout do canal</option>
            <option value="views_per_day">Ordenar: Views/dia</option>
            <option value="views">Ordenar: Total views</option>
            <option value="score">Ordenar: Score</option>
            <option value="published_at">Ordenar: Publicação</option>
            <option value="collected_at">Ordenar: Coleta</option>
          </select>
        </div>

        <button onClick={() => setFilter("sort_order", filters.sort_order === "asc" ? "desc" : "asc")} className="flex h-10 w-10 items-center justify-center rounded-lg border border-slate-800 bg-slate-950/50 text-slate-400 hover:bg-slate-900 hover:text-slate-200" title="Alternar ordenação">
        ...
        <button onClick={resetFilters} className="flex h-10 w-10 items-center justify-center rounded-lg border border-slate-800 bg-slate-950/30 text-slate-500 hover:bg-slate-900 hover:text-slate-300" title="Limpar filtros"><RotateCcw className="h-4 w-4" /></button>
      </div>

      <div className={cn("grid gap-4 border-t border-slate-800/60 pt-4 md:grid-cols-5", showAdvanced ? "grid" : "hidden")}>
        <div className="space-y-1.5">
          <label className="text-xs font-semibold uppercase tracking-wider text-slate-500">Fonte</label>
          <select value={filters.source} onChange={(e) => setFilter("source", e.target.value)} className="w-full rounded-lg border border-slate-800 bg-slate-950/80 px-3 py-2 text-sm text-slate-300 outline-none focus:border-indigo-500">
            <option value="Todos">Todas</option><option value="youtube">YouTube</option><option value="google_news">Google News</option>
          </select>
        </div>
        <div className="space-y-1.5">
          <label className="text-xs font-semibold uppercase tracking-wider text-slate-500">Tipo</label>
          <select value={filters.content_type} onChange={(e) => setFilter("content_type", e.target.value)} className="w-full rounded-lg border border-slate-800 bg-slate-950/80 px-3 py-2 text-sm text-slate-300 outline-none focus:border-indigo-500">
            <option value="Todos">Todos</option><option value="video">Vídeo</option><option value="article">Artigo</option>
          </select>
        </div>
        <div className="space-y-1.5">
        ...
        <div className="space-y-1.5">
          ...
          <div className="relative">
            <input type="number" min="0" step="0.1" value={filters.min_performance_ratio || ""} onChange={(e) => setFilter("min_performance_ratio", e.target.value ? parseFloat(e.target.value) : 0)} placeholder="2.0" className="w-full rounded-lg border border-slate-800 bg-slate-950/80 px-3 py-2 pr-8 text-sm text-slate-300 outline-none focus:border-indigo-500" />
            <span className="absolute right-3 top-1/2 -translate-y-1/2 text-xs text-slate-600">×</span>
          </div>
          <p className="text-[10px] text-slate-600">Comparado à mediana recente do canal.</p>
        </div>
      </div>
    </div>
  );
